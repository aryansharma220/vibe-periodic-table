...
const ElectronOrbit = ({ radius, electronCount, orbitColor, electronColor, speed = 1, rotation = [0, 0, 0] }) => {
  ...
  useFrame((state) => {
    if (orbitRef.current && electrons.current.length > 0) {
      ...
      electrons.current.forEach((electron, i) => {
        if (orbitRef.current.children[i+1]) { // +1 to skip the orbit ring which is the first child
  ...
  return (
    <group ref={orbitRef}>
      ...
      <mesh rotation={[Math.PI/2, 0, 0]}>
        ...
        <meshBasicMaterial color={orbitColor} side={THREE.DoubleSide} transparent opacity={0.5} />
      </mesh>
        {/* Electrons */}
...
const Nucleus = ({ protonCount, neutronCount, elementColor }) => {
  ...
  const nucleusSize = Math.max(0.8, Math.min(1.5, Math.pow(totalParticles / 10, 1/3)));
...
// Main Atom component
const Atom = ({ element, elementColor }) => {  const electronCount = element.number;
  ...
  const getElectronShells = () => {
    ...
    for (let n = 0; n < maxElectronsPerShell.length && remainingElectrons > 0; n++) {
      const shellElectrons = Math.min(maxElectronsPerShell[n], remainingElectrons);
  ...
  return (
    <>
      ...
      <pointLight position={[0, 0, 0]} intensity={20} color={elementColor} distance={10} />
      ...
      {/* Electron shells */}
      {electronShells.map((electronCount, i) => {      // Calculate orbit radius with increasing distance from nucleus
        // Using improved scaling for shell distances that better approximates quantum mechanical orbitals
        // and provides more consistent visual spacing        // Base distance carefully calculated to ensure proper spacing between nucleus and first shell
        // For the first shell (i=0), the nucleusScaleFactor has more impact
        // For outer shells, the progressive spacing takes over
...
function Atom3DModel({ element, elementColor, size = 300 }) {
  ...
  return (
    <div style={{ width: '100%', height: size, position: 'relative' }}>      <Canvas 
      ...
      {showControls && (
        ...
        >
          <div style={{ marginBottom: '8px', fontWeight: 'bold' }}>Interactive 3D Model</div>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ 
            ...
            height: '8px', 
            borderRadius: '50%', 
            background: new THREE.Color(elementColor).offsetHSL(0, -0.4, -0.2).getStyle(),
            marginRight: '5px'
          }}></div>
          <div>Neutrons</div>
        </div>

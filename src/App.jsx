...
function ShootingStar() {
  ...
  const gradient = colorChoices[Math.floor(Math.random() * colorChoices.length)];
...
function StarParticle({ index }) {
  ...
  const color = colors[Math.floor(Math.random() * colors.length)].replace('OPACITY', baseOpacity.toString());
    ...
    <div 
      ...
      style={{
        ...
        boxShadow: isExtraShiny
          ? `0 0 ${size * 2}px ${color}, 0 0 ${size * 4}px ${color}, 0 0 ${size * 6}px rgba(255,255,255,0.3)`
          : `0 0 ${size * glowIntensity}px ${color}`,
        animation: isPulse 
          ? `float-${index % 3} ${speed}s infinite ease-in-out ${delay}s, ${isExtraShiny ? 'shine' : 'pulse'} ${Math.random() * 3 + 2}s infinite ease-in-out ${delay}s` 
  ...
  return (
    <div 
      ...
      style={{
        ...
        top: `${initialY}%`,
        backgroundColor: '#fff',
        boxShadow: `0 0 ${size}px #fff, 0 0 ${size * 2}px rgba(255,255,255,0.8), 0 0 ${size * 3}px rgba(255,255,255,0.6)`,
